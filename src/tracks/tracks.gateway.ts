 ...
 * keep old clients working. Connect with `?source=drone_rid` (or `?source=adsb`)
 * to receive only that source, otherwise all updates arrive.
 */
@WebSocketGateway({
  cors: { origin: true },
...
export class TracksGateway {
  ...
  constructor(private readonly tracks: TrackStoreService) {
    ...
    this.tracks.events.on(
      'remove',
      (r: { source: 'adsb' | 'drone_rid'; id: string; at: number }) => {
  ...
  handleConnection(client: Socket): void {
    ...
    if (source === 'adsb' || source === 'drone_rid') {
  ...
  handleList(@MessageBody() payload?: { source?: string }): {
    ...
    const source =
      payload?.source === 'adsb' || payload?.source === 'drone_rid'
        ? payload.source
        : undefined;
